...
function Post({ post }) {
    ...
    const deletePostHandler = async () => {
        try {
            const res = await axios.delete(`http://localhost:3000/api/v1/post/deletePost/${post?._id}`, {
    ...
    const likeOrDislikeHandler = async () => {
        try {
            const likeStatus = liked ? "dislike" : "like"
            const res = await axios.get(`http://localhost:3000/api/v1/post/${post?._id}/${likeStatus}`, {
                withCredentials: true
            })
            if (res.data.success) {
        ...
        <div className='my-8 w-full max-w-sm mx-auto'>
            <div className='flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                    <Avatar>
                        <AvatarImage src={post.author?.profilePicture} alt="@InstaPound" />
                ...
                <Dialog>
                    ...
                    <DialogContent className="flex flex-col items-center text-center text-sm ">
                        <Button variant="ghost" className="cursor-pointer w-fit text-[#ED4596] font-bold">Unfollow</Button>
            ...
            <img className='rounded-sm my-2 w-full aspect-square object-cover' src={post.image} alt="@InstaPound" />
